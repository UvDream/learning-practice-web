...
RegCode.prototype.arc = function() {
  for (let i = 0; i < this.dotNum; i++) {
    ...
    let colors = this.getColor(this.backgroundColor);
    this.paint.fillStyle = `rgba(${colors[0]}, ${colors[1]}, ${
      colors[2]
    }, 0.8)`;
    this.paint.fill();
  }
...
// 绘制线条
RegCode.prototype.line = function() {
  // 绘制线条
  for (let i = 0; i < this.lineNum; i++) {
    ...
    this.paint.strokeStyle = `rgba(${colors[0]}, ${colors[1]}, ${
      colors[2]
    }, 0.8)`;
    // 指定绘制路径
...
RegCode.prototype.font = function() {
  ...
  for (let i = 0; i < this.len; i++) {
    ...
    this.paint[colorStyle] = `rgba(${colors[0]}, ${colors[1]}, ${
      colors[2]
    }, 0.8)`;
    // 开始绘制
    this.paint.save();
